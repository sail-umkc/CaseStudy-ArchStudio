...
public class VariantsEventListener implements IArchipelagoEventListener{
	...
	*/
	...
	public static IBNAWorld createVariantWorld(ArchipelagoServices AS, ObjRef xArchRef, ObjRef variantTypeRef, boolean addEditingBehaviors){
		...
		for(int i = 0; i < variantRefs.length; i++){
			ObjRef typeRef = XadlUtils.resolveXLink(AS.xarch, variantRefs[i], "variantType");
			if(typeRef != null){
				...
				if(isAppropriateType){
					BoxAssembly typeAssembly = BoxAssembly.create(m, null);

					String description = XadlUtils.getDescription(AS.xarch, typeRef);
					if(description == null) description = "[No Description]";
					
					typeAssembly.getBoxGlassThing().setBoundingBox(
						(DefaultCoordinateMapper.DEFAULT_WORLD_WIDTH / 2) + offset, (DefaultCoordinateMapper.DEFAULT_WORLD_HEIGHT / 2), 150, 100);
					offset += 175;
					
					typeAssembly.getBoxedLabelThing().setText(description);
					typeAssembly.getBoxGlassThing().setToolTip(description);
					...
					switch(typeKind){
						case COMPONENT_TYPE:
							typeAssembly.getBoxThing().setColor(TypesMapper.getDefaultComponentTypeColor(AS));
							typeAssembly.getBoxBorderThing().setCount(2);
							break;
						case CONNECTOR_TYPE:
							typeAssembly.getBoxThing().setColor(TypesMapper.getDefaultConnectorTypeColor(AS));
