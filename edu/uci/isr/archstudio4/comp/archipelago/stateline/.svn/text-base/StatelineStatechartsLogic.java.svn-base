...
import edu.uci.isr.archstudio4.comp.archipelago.statecharts.StatechartsMapper;
...
public class StatelineStatechartsLogic extends AbstractThingLogic implements IBNAMenuListener{
	...
	public void fillMenu(IBNAView view, IMenuManager m, int localX, int localY, final IThing t, int worldX, int worldY){
		...
		if((t != null) && ((t instanceof BoxGlassThing) || (t instanceof DistinguishedStateGlassThing))){
			IBNAModel bnaModel = view.getWorld().getBNAModel();
			...
			if(selectedThings.length <= 1){
				...
				if(t instanceof BoxGlassThing){
					IThing pt = bnaModel.getParentThing(t);
					if((pt != null) && (StatechartsMapper.isStateAssemblyRootThing(pt))){
						assembly = BoxAssembly.attach(bnaModel, (IHasAssemblyData)pt);
					}
				}
				else if(t instanceof DistinguishedStateGlassThing){
					IThing pt = bnaModel.getParentThing(t);
					if((pt != null) && (StatechartsMapper.isStateAssemblyRootThing(pt))){
						assembly = DistinguishedStateAssembly.attach(bnaModel, (IHasAssemblyData)pt);
					}
				}
				if(assembly != null){
					final IAssembly fassembly = assembly;
					IAction runStatelineSelectionAction = new Action("Run Stateline Selection..."){
						public void run(){
							runStatelineSelection(t, fassembly);
						}
					};
					m.add(runStatelineSelectionAction);
	...
	protected void runStatelineSelectionForStatechart(ObjRef statechartRef){
		...
		ObjRef startingPointRef = XadlSelectorDialog.showSelectorDialog(
			shell, "Select Selection Start Point", AS.xarch, AS.resources, 
			xArchRef, XadlTreeUtils.STRUCTURE | XadlTreeUtils.DOCUMENT | XadlTreeUtils.COMPONENT_TYPE | XadlTreeUtils.CONNECTOR_TYPE, 
				...
				for(int i = 0; i < stateRefs.length; i++){
					...
					if(stateID != null){
						...
						if(processedRef != null){
							...
							if(stateDescription == null) stateDescription = "state" + i;
