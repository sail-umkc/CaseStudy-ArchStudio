...
public class StatelineStructureLogic extends AbstractThingLogic implements IBNAMenuListener{
	...
	public void fillMenu(final IBNAView view, IMenuManager m, int localX, int localY, IThing t, int worldX, int worldY){
		...
		List<IAssembly> targetAssemblyList = new ArrayList<IAssembly>();
		
		for(int i = 0; i < selectedThings.length; i++){
			if(selectedThings[i] instanceof BoxGlassThing){
				IThing pt = bnaModel.getParentThing(selectedThings[i]);
				if((pt != null) && StructureMapper.isBrickAssemblyRootThing(pt)){
					BoxAssembly assembly = BoxAssembly.attach(bnaModel, (IHasAssemblyData)pt);
					targetAssemblyList.add(assembly);
				}
			}
			else if(selectedThings[i] instanceof StickySplineGlassThing){
				IThing pt = bnaModel.getParentThing(selectedThings[i]);
				if((pt != null) && StructureMapper.isLinkAssemblyRootThing(pt)){
					StickySplineAssembly assembly = StickySplineAssembly.attach(bnaModel, (IHasAssemblyData)pt);
					targetAssemblyList.add(assembly);
				}
			}
		}
		
		final IAssembly[] targetAssemblies = targetAssemblyList.toArray(new IAssembly[targetAssemblyList.size()]);
		...
		IAction setAllowedStatesAction = new Action("Select Stateline States"){
			public void run(){
				selectStatelineStates(view, targetAssemblies);
			}
		};
	...
	protected void selectStatelineStates(IBNAView view, IAssembly[] assemblies){
		...
		if(c != null){
			...
			ObjRef[] stateRefs = XadlSelectorDialog.showSelectorDialog(
				c.getShell(), "Select Applicable States", AS.xarch, AS.resources, 
				xArchRef, XadlTreeUtils.STATECHART | XadlTreeUtils.STATE, 
				XadlTreeUtils.STATE, true);
