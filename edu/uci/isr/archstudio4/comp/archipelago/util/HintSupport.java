...
public class HintSupport implements IPropertyCoder{
	...
	public Object readProperty(ArchipelagoServices AS, ObjRef xArchRef, ObjRef eltRef, String name){
		...
		for(int i = 0; i < propertyRefs.length; i++){
			String n = (String)AS.xarch.get(propertyRefs[i], "name");
			if((n != null) && (n.equals(name))){
				ObjRef propertyValueRef = (ObjRef)AS.xarch.get(propertyRefs[i], "value");
				try{
	...
	public static ObjRef getArchipelagoHintsBundleRef(ArchipelagoServices AS, ObjRef xArchRef){
		...
		for(int i = 0; i < bundleRefs.length; i++){
			if(AS.xarch.isInstanceOf(bundleRefs[i], "hints3#HintBundle")){
				String maintainer = (String)AS.xarch.get(bundleRefs[i], "maintainer");
				String version = (String)AS.xarch.get(bundleRefs[i], "version");
	...
	/*
	public static ObjRef findHintedElementRef(ArchipelagoServices AS, ObjRef xArchRef, ObjRef bundleRef, String targetID){
		ObjRef[] topLevelRefs = AS.xarch.getAll(bundleRef, "hintedElement");
		String targetHref = "#" + targetID;
		for(int i = 0; i < topLevelRefs.length; i++){
			ObjRef targetRef = (ObjRef)AS.xarch.get(topLevelRefs[i], "target");
			if(targetRef != null){
				String href = XadlUtils.getHref(AS.xarch, targetRef);
				if((href != null) && (href.equals(targetHref))){
	...
	*/
	...
	public static ObjRef findChildHintedElementRef(ArchipelagoServices AS, ObjRef xArchRef, ObjRef rootRef, ObjRef targetRef){
		...
		for(int i = 0; i < subRefs.length; i++){
			ObjRef tRef = XadlUtils.resolveXLink(AS.xarch, subRefs[i], "target");
	...
	public static ObjRef findHintedElementRef(ArchipelagoServices AS, ObjRef xArchRef, ObjRef rootRef, ObjRef targetRef){
		...
		ObjRef[] subRefs = AS.xarch.getAll(rootRef, "hintedElement");
		for(int i = 0; i < subRefs.length; i++){
			ObjRef matchRef = findHintedElementRef(AS, xArchRef, subRefs[i], targetRef);
			if(matchRef != null){
				return matchRef;
			}
		}
