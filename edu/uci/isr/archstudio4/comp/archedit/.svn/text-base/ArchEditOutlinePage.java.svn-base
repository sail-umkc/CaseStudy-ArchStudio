...
public class ArchEditOutlinePage extends AbstractArchstudioOutlinePage{
	...
	public ArchEditOutlinePage(XArchFlatInterface xarch, ObjRef xArchRef, IResources resources){
	...
	protected void setupDragAndDrop(){
		DropTarget target = new DropTarget(getTreeViewer().getTree(), DND.DROP_MOVE | DND.DROP_COPY
			| DND.DROP_DEFAULT);
	...
	protected class ArchEditNode{
		...
		public ArchEditNode[] getChildren(){
			...
			for(int i = 0; i < properties.length; i++){
				if(properties[i].getMetadataType() == IXArchPropertyMetadata.ELEMENT){
					String eltName = properties[i].getName();
					ObjRef childRef = (ObjRef)xarch.get(ref, eltName);
					if(childRef != null){
						l.add(new ArchEditNode(childRef));
					}
				}
				else if(properties[i].getMetadataType() == IXArchPropertyMetadata.ELEMENT_MANY){
					String eltName = properties[i].getName();
					ObjRef[] childRefs = xarch.getAll(ref, eltName);
					for(int j = 0; j < childRefs.length; j++){
						l.add(new ArchEditNode(childRefs[j]));
					}
				}
			}
		...
		public boolean hasChildren(){
			for(int i = 0; i < properties.length; i++){
				...
				if(properties[i].getMetadataType() == IXArchPropertyMetadata.ELEMENT_MANY){
					return true;
				}
			}
			return false;
	...
	protected void fillContextMenu(IMenuManager menuMgr){
		...
		if(selectedRefs.length == 0){
			...
			ObjRef ref = selectedRefs[0];
			ObjRef parent = xarch.getParent(ref);
			if(parent == null){
			...
			public void run(){
				...
				try{
					...
					try{
						...
						xarch.remove(parentRef, "Object", fref);
					}
				}
