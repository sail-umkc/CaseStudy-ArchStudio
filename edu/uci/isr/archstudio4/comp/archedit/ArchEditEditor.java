...
public class ArchEditEditor extends AbstractArchstudioEditor{
	...
	public ArchEditEditor(){
		...
		setBannerInfo(((ArchEditMyxComponent)comp).getIcon(), 
			"Syntax-Directed Architecture Editor");
		setHasBanner(true);
	}
	
	protected AbstractArchstudioOutlinePage createOutlinePage(){
		return new ArchEditOutlinePage(xarch, xArchRef, resources);
	}
	...
	public void createEditorContents(Composite parent){
		...
		if((selectedRefs == null) || (selectedRefs.length == 0)){
			...
			for(int i = 0; i < selectedRefs.length; i++){
				...
				IXArchTypeMetadata typeMetadata = xarch.getTypeMetadata(selectedRefs[i]);
				StringBuffer headerLine = new StringBuffer();
				headerLine.append(SystemUtils.capFirst(xarch.getElementName(selectedRefs[i])));
				headerLine.append(": ");
				headerLine.append(XArchMetadataUtils.getTypeName(typeMetadata.getType()));
				lElement.setText(headerLine.toString());
				
				if(xarch.isInstanceOf(selectedRefs[i], "instance#XMLLink")){
					createDragSourceComposite(parent, selectedRefs[i]);
				}
			...
			for(IXArchPropertyMetadata propMetadata : typeMetadata.getProperties()){
				if (propMetadata.getMetadataType() == IXArchPropertyMetadata.ATTRIBUTE){
					l.add(new String[]{
						...
						(String)xarch.get((ObjRef)inputElement, propMetadata.getName())
					});
				}
			}
			return l.toArray();
	...
	class ViewCellModifier implements ICellModifier{
		...
		public void modify(Object element, String property, Object value){
			...
			if(element instanceof String[]){
				...
				if(elts[1] != null) oldValue = elts[1].toString();
